...
export const EditChannelModal = () => {
    ...
    const { channelData, channelType } = data;
    ...
    useEffect(() => {
        ...
        if (channelData) {
            form.setValue("name", channelData.name);
        }
    }, [channelType, channelData, form]);
    ...
    const onSubmit = async (values: z.infer<typeof channelCreationProps>) => {
        try {
            const response = await axios.patch(`/api/servers/${server.id}/channels/${channelData?.id}`, values);
    ...
    return (
        <Dialog open={isModalOpen} onOpenChange={handleClose}>
            <DialogContent className="bg-[#f1f1f1] text-black py-8 px-6 dark:bg-[#1f1f23] dark:text-white">
                <DialogHeader>
                    <DialogTitle className="text-3xl font-semibold text-center">Edit Channel</DialogTitle>
                </DialogHeader>
